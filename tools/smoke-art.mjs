...
try {
  await page.goto(`http://127.0.0.1:${PORT}/dist/artifact-test.html`, { waitUntil: 'load' });
  ...
  const st = await page.evaluate(() => (window.__APP__ && window.__APP__.selfTest ? window.__APP__.selfTest() : null))
    .catch(() => null);
  if (st) ok('vlastní test v appce (' + st.pass + '/' + (st.pass + st.fail) + ', ' + st.skip + ' přeskočeno)', st.fail === 0);
